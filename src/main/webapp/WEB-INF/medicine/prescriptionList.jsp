				...
				<table class="listTable listTable2 center">
				    ...
				    <tbody>
				        <c:forEach var="prescription" items="${prescriptions}" varStatus="status">
				            <tr onclick="window.location.href='${pageContext.request.contextPath}/medicine/prescriptionDetail.do?id=${status.index}'">
				                <td>${status.index + 1}</td>
				                <td><a href="${pageContext.request.contextPath}/medicine/prescriptionDetail.do?id=${status.index}">${prescription.resMenufactureDate}</a></td>
